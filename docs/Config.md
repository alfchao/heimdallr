...
若需要通过代理或自定义 API 地址访问某些推送服务（如在内网环境或使用代理转发），可以配置以下全局变量：

| 变量名                      | 默认值                                                   | 变量说明                                          |
| --------------------------- | -------------------------------------------------------- | ------------------------------------------------- |
| `TELEGRAM_BASE_URL`         | `https://api.telegram.org/bot`                           | Telegram API 基础地址（协议+域名+路径）           |
| `WECOM_WEBHOOK_BASE_URL`    | `https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=` | 企业微信 WebHook 基础地址（协议+域名+路径+key=） |
| `WECOM_APP_BASE_URL`        | `https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token=` | 企业微信应用消息 API 基础地址（协议+域名+路径+access_token=） |
| `WECOM_AUTH_BASE_URL`       | `https://qyapi.weixin.qq.com/cgi-bin/gettoken`          | 企业微信获取 token API 地址（协议+域名+路径）    |

**使用场景示例：**

```bash
# 使用代理访问 Telegram
TELEGRAM_BASE_URL=http://proxy.example.com:8080/telegram/api/bot

# 使用自定义域名访问企业微信
WECOM_WEBHOOK_BASE_URL=https://wecom.company.internal/cgi-bin/webhook/send?key=
WECOM_APP_BASE_URL=https://wecom.company.internal/cgi-bin/message/send?access_token=
WECOM_AUTH_BASE_URL=https://wecom.company.internal/cgi-bin/gettoken
```

## 通知分组和渠道配置

本服务有分组和通知渠道的概念。通知渠道是最小的通知单位，可以通过某一个应用及其凭据发送通知；分组为多个通知渠道的组合，可以通过一个 [...]

见以下最小例子，其定义了一个名为 `group_1` 的分组，并具有一个对应的 `token`，启用了 `channel_1`, `channel_2`, `channel_3` 三个通知渠道。其中 `channel_1` ··[...]
...
| <GROUP_NAME>_TOKEN            | （选填）分组的 Token，对应请求时使用的 {key} 。建议使用随机生成的字母和数字，不可以带 `/`。如果该分组不直接使用，��不设置。
...
| `TELEGRAM_TOKEN`        | Telegram  | Telegram 的 Token，见 [这里](https://github.com/pppscn/SmsForwarder/wiki/2.%E5%8F%91%E9%80%81%E9%80%9A%E9%81%93#tele%E6%9C%BA%E5%99%A8%E4%BA%BA%E7%A7%[...]
...
| `DINGTALK_SECRET`       | 钉钉      | 钉钉的 secret，通过加签方式保护机器人。与关键词只能二选一，推荐使用签名，见 [这里](https://open.dingtalk.com/document[...]
