...
| 变量名                | 默认值                     | 变量说明                                       |
| --------------------- | -------------------------- | ---------------------------------------------- |
| `TELEGRAM_BASE_URL`   | `https://api.telegram.org` | Telegram API 基础地址（仅协议+域名，路径自动拼接） |
| `WECOM_BASE_URL`      | `https://qyapi.weixin.qq.com` | 企业微信 API 基础地址（仅协议+域名，路径自动拼接，WebHook/App/Auth 共用） |

**使用场景示例：**

```bash
# 使用代理访问 Telegram
TELEGRAM_BASE_URL=http://proxy.example.com:8080/telegram/api

# 使用自定义域名访问企业微信
WECOM_BASE_URL=https://wecom.company.internal
```
